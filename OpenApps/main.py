...
def readDicomImages(in_dir):
    Logger.info("Reading dicom images started")
    t1_img = None
    flair_img = None
    ...
    return t1_img, flair_img, flair_series_reader
    

def readImagesIntoNPArray(t1_img, flair_img):
...
def main(argv):
    ...
    t1_img, flair_img, flair_series_reader = readDicomImages(args.inputFolder)
    t1_data, flair_data = readImagesIntoNPArray(t1_img, flair_img)
    computed_data = computeResult(t1_data, flair_data)
    showResult(t1_data,computed_data)
